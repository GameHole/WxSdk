...
public static class XmlSetter 
{
    ...
    public static void SetXml()
    {
        ...
        var settedNode = FindNode(document, "/manifest/application/activity", "android:name", $"{Application.identifier}.wxapi.WXEntryActivity");
        if (settedNode != null) return;
        var node = FindNode(document, "/manifest/application/activity", "android:name", "package.wxapi.WXEntryActivity");
        if (node != null)
        {
        ...
        else
        {
            ...
            SetPermission(document, "android.permission.INTERNET");
            SetPermission(document, "android.permission.ACCESS_WIFI_STATE");
            SetPermission(document, "android.permission.ACCESS_NETWORK_STATE");
        }
        document.Save(plugpath);
